...
if __name__ == "__main__":
    ...
    test_h5_file = os.path.join(h5_path, "{}_data.h5".format("train"))     
    ...
    if is_start_making_h5 and \
        ...
        if is_load_encoded:
            ...
            train_loader = DataLoader(train_dataset, batch_size=50, shuffle=False,collate_fn=collate_fn_loadencoded, num_workers=4)
        else:
        ...
        with h5py.File(test_h5_file, 'w') as f:
            ds_cf = f.create_dataset('clone_fractions', shape=(len(train_dataset), ), dtype=np.float32)
            ds_labels = f.create_dataset('labels', shape=(len(train_dataset), ), dtype=np.int32)
            ds_patient_id = f.create_dataset('patient_id', shape=(len(train_dataset), ), dtype=np.int32)
            ds_sequence_id = f.create_dataset('sequence_id', shape=(len(train_dataset), ), dtype=np.int32)
            ds_cdr3_seq = f.create_dataset('cdr3_seq', shape=(len(train_dataset), ), dtype=h5py.string_dtype(encoding='utf-8'))
            ds_esm = f.create_dataset('cdr3_esm_encoded', shape=(len(train_dataset), 960), dtype=np.float32)
            start_idx = 0
            ...
            with torch.no_grad():
                ...
                for sid, fid, cdr3, label, cloneFraction in tqdm.tqdm(train_loader, \
                desc="embedding train set", total=len(train_loader)):
                    ...
                    ds_cf[start_idx:start_idx+batch_size] = np.array(cloneFraction, dtype=np.float32)
                    ds_labels[start_idx:start_idx+batch_size] = np.array(label, dtype=np.int32)
                    ds_patient_id[start_idx:start_idx+batch_size] = np.array(fid, dtype=np.int32)
                    ds_sequence_id[start_idx:start_idx+batch_size] = np.array(sid, dtype=np.int32)
                    ds_cdr3_seq[start_idx:start_idx+batch_size] = np.array(cdr3, dtype=h5py.string_dtype(encoding='utf-8'))
                    ds_esm[start_idx:start_idx+batch_size] = np.array(Esm_emb, dtype=np.float32)
                    f.flush()
                ...
                #     # ds_esm = f.create_dataset('cdr3_esm_encoded', shape=(n_samples, 960), dtype=np.float32)
                #     # start_idx = 0
